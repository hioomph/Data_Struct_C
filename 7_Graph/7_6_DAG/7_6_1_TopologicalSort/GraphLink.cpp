...
void TopologicalSort(GraphLink *g)
{
    ...
    int *count = (int*)malloc(sizeof(int)*n);  // 初始化count数组，存储顶点及其入度信息
    assert(count != NULL);
    for(int i=0; i<n; ++i)
    {
        count[i] = 0;
    ...
    for(int i=0; i<n; ++i)
    {
        p = g->NodeTable[i].adj;  // p指向存放当前顶点边信息的邻接表
        while(p != NULL)
        {
            count[p->dest]++;  // 在count数组中，对应有入度的顶点位置++
            p = p->link;  // p指向下一个顶点
        }
    }
    ...
    for(int i=0; i<n; ++i)
    {
        ...
        {
            // 模拟入栈
            count[i] = top;  // count[i]保存了上一个入度为0的顶点的下标   
            top = i;  // top指向顶点i（即入度为0的栈顶元素）
        }
    }
    ...
    for(int i=0; i<n; ++i)
    {
        if(top == -1)
        {
            printf("网络中有回路，无法进行拓扑排序！\n");
            return;
        }
        else
        {
            // 模拟出栈
            v = top;  // v指向栈顶元素（即此时度为0的顶点）
            top = count[top];  // 相当于出栈顶点v的操作
            printf("%c-->", g->NodeTable[v]);
            // 求解v的邻接顶点
            w = GetFirstNeighbor(g, g->NodeTable[v].data);
            while(w != -1)
            {
                // 删去对应的边后若入度为0，则对顶点w进行入栈操作
                if(--count[w] == 0)
                {
                    // 入栈
                    count[w] = top;
